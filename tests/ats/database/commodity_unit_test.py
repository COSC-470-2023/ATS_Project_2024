...
#         data = historical_commodity_insert.load_output_file("../../test_files/static_test_files/static_commodities_historical.json")
#         with connect.connect() as conn:
#             for entry in data:
#                 commodity_id = historical_commodity_insert.get_commodity_id(entry, conn)
#                 historical_commodity_insert.execute_insert(conn, entry, commodity_id)
#             result = conn.execute(sqlalchemy.text(f"select id from `commodities` where symbol = 'HGUSD'"))
...
#             result = conn.execute(sqlalchemy.text(f"select volume from `historical_commodity_values` where commodity_id = '{retrieved_commodity_id}' and date = '2023-11-23'"))
#             row = result.one_or_none()
...
#         data = realtime_commodity_insert.load_output_file("../../test_files/static_test_files/static_commodities_realtime.json")
#         with connect.connect() as conn:
#             for entry in data:
#                 commodity_id = realtime_commodity_insert.get_commodity_id(entry, conn)
#                 realtime_commodity_insert.execute_insert(conn, entry, commodity_id)
#             result = conn.execute(sqlalchemy.text(f"select id from `commodities` where symbol = 'HGUSD'"))
#             row = result.one_or_none()
#             self.assertIsNotNone(row, msg="The insertion script did not insert into commodities as expected, as the expected row was not found in the database.")
#             retrieved_commodity_id = row[0]
#             result = conn.execute(sqlalchemy.text(f"select volume from `realtime_commodity_values` where commodity_id = '{retrieved_commodity_id}'"))
#             row = result.one_or_none()
