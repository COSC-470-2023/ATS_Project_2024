# Helper function to convert historical stock list queries from FMP API into system usable output
# Supports static files and testing
# Pass the file name including file extension as an argument via script or cli
# Current version expects single additional argument indicating file name to be loaded from static tests file dir
...
def load_json(file_name):
    file_path = 'tests/integration/static_test_files/' + file_name
    try:
...
def write_json(json_content, file_name):
    output_dir = "tests/integration/static_test_files/"
        ...
        if hist == 'historicalStockList':
            ...
            for hist_entry in raw_json_output[hist]:
                ...
                for hist_values in hist_entry['historical']:
                    ...
                    opening = hist_values['open']
                    high = hist_values['high']
                    low = hist_values['low']
                    close = hist_values['close']
                    adj_close = hist_values['adjClose']
                    volume = hist_values['volume']
                    unadjusted_volume = hist_values['unadjustedVolume']
                    change = hist_values['change']
                    change_percent = hist_values['changePercent']
                    vwap = hist_values['vwap']
                    change_over_time = hist_values['changeOverTime']
                    ...
                    modified_json_output.append({
                        ...
                        "_historical_open": opening,
                        "_historical_high": high,
                        "_historical_low": low,
                        "_historical_close": close,
                        "_historical_adjClose": adj_close,
                        "_historical_volume": volume,
                        "_historical_unadjustedVolume": unadjusted_volume,
                        "_historical_change": change,
                        "_historical_changePercent": change_percent,
                        "_historical_vwap": vwap,
                        "_historical_changeOverTime": change_over_time
                    })
        else:
            continue
...
def get_symbol_name(stock_symb):
    ...
    symbol_names = {"AAPL": "Apple Inc.", "NDAQ": "Nasdaq, Inc.", "META": "Meta Platforms, Inc.",
                    "GOOG": "Alphabet Inc.",
                    ...
                    "^RUA": "Russell 3000", "^FTSE": "FTSE 100", "HGUSD": "Copper", "CLUSD": "Crude Oil",
                    "GCUSD": "Gold Futures",
