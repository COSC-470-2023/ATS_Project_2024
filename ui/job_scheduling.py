...
def change_schedule():
    ...
    cron_time_input = f"{minute} {hour} * * {day_of_week_cron}"

    if job_script_name in cron_jobs: #if the cron jobs file has a setting for the selected data collection job
        cron_jobs[job_script_name]['time'] = time   #set the cron job time to be what the front end sends us... THIS NEEDS TO BE CHANGED WITH THE UPDATED FRONTEND
        script_path = cron_jobs[job_script_name]['script_path'] 
        #get the script path to write to the actual cron jobs later
    else:
        return flash("it dont exist fam", category='error') #replace with better error msgs or logging later, also dont think this works
